    ...
    class Program
    {
        ...
        */
        ...
        private static string RemoveVowels(string s)
        {
            try
            {
                ...
                {
                    s = s.Replace(c.ToString(), String.Empty);//it replaces the exisiting vowel and remove it
                }
       ...
       */
        ...
        private static bool ArrayStringsAreEqual(string[] bulls_string1, string[] bulls_string2)
        {
            try
            {
                string str1 = "";// creating a string place holder
                string str2 = "";
                for (int i = 0; i < bulls_string1.Length; i++)
                {
                    str1 = str1 + bulls_string1[i]; // combining all the strings in the array1 
                }
                for (int i = 0; i < bulls_string2.Length; i++)
                {
                    str2 = str2 + bulls_string2[i]; // combining all the strings in the array2
                }
        ...
        */
        ...
        private static int SumOfUnique(int[] bull_bucks)
        {
            try
            {
                int add = 0;
                int sum = 0;
                for (int i = 0; i < bull_bucks.Length; i++) // looping to check the occurences
                {
                    for (int j = 0; j < bull_bucks.Length; j++)
                    {
                        if(bull_bucks[i] == bull_bucks[j])
                        {
                            add = add + 1; // occurences of an element
                        }
                    }
                    if(add == 1) // if it is unique it gets added to the sum
                    {
        ...
        */
        ...
        private static int DiagonalSum(int[,] bulls_grid)
        {
            ...
            {
                int diag = 0;
                int dim = bulls_grid.GetLength(0);// since it's a square matrix
                for (int i = 0; i<dim; i++)// looping through the 2D array
                {
                    for (int j = 0; j < dim; j++)
                    {
        ...
        */
        ...
        private static string RestoreString(string bulls_string, int[] indices)
        {
            try
            {
                ...
                {
                    dict.Add(indices[i], bulls_string[i]); // adding key, values to the dictionary
                }
        ...
        private static string ReversePrefix(string bulls_string6, char ch)
        {
            try
            {
                ...
                for (int i = 0; i < bulls_string6.Length; i++)
                {
                    ...
                    {
                        index = i;
                        break; // if the first instance of the character is found, loop breaks
                    }
                }
                if(index == 0) // if the char doesn't exist, it returns the whole string
                {
                    prefix_string = bulls_string6;
                    return prefix_string;
                }

                for (int i = index; i >=0; i--) // reversing from the char index
                {
                    res = res + bulls_string6[i];
                }
                
                if(res.Length != bulls_string6.Length)
                {
                    prefix_string = res + bulls_string6.Substring(index + 1, bulls_string6.Length - index-1);// adding reversed string to the rest
                    return prefix_string;
                }
                prefix_string = res; // if char is the last index it should return the reversed string itself
                return prefix_string;
            }
            catch (Exception)
            {
